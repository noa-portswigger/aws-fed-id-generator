...
public class TokenGenerator {
    ...
    public TokenGenerator(AwsCredentialsProvider credentialsProvider) {
        this.region = Region.US_EAST_1;
        this.credentialsProvider = credentialsProvider != null ? credentialsProvider : DefaultCredentialsProvider.builder().build();
    }
    ...
    /**
     * Generate a token used by a third-party system to authenticate an AWS role. The format of this token conforms
     * to the subject token expected by the Google Cloud Platform (GCP) Secure Token Service (STS) /v1/token endpoint.

     * The format is described
     * <a href="https://cloud.google.com/iam/docs/workload-identity-federation-with-other-clouds">here</a>.
     * The section with python code under "Advanced Scenarios" is particularly helpful. The reason for conforming to
     * this format is mainly to piggyback on the security analysis that no doubt went into the GCP Identity federation
     * setup.
     *
     * @param audience When using with GCP, this is the workload identity pool audience. In other use cases, use
     *                 some string such as hostname that uniquely identifies the authenticating party. The purpose
     *                 of this value is to avoid replay attacks where a token created for a different audience
     *                 is being re-used to authenticate.
    ...
    public String generate(String audience) {
        try {
            ...
            @SuppressWarnings("deprecation") Aws4Signer signer = Aws4Signer.create();
